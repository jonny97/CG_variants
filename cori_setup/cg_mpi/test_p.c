...
int main(int argc, char *argv[])
{
    ...
    printf("start at %02d:%02d:%02d\n", current_time->tm_hour, current_time->tm_min, current_time->tm_sec);
        ...
        //int vec_len_per_proc = M/n_proc; 
        int size_rank = M/n_proc; 
        double *x = (double *) malloc(size_rank * sizeof(double));
        double *r = (double *) malloc(size_rank * sizeof(double));
        double *p = (double *) malloc(size_rank * sizeof(double));
        double *b = (double *) malloc(size_rank * sizeof(double));
        double *sol = (double *) malloc(size_rank * sizeof(double));
        double *Ap = (double *) malloc(size_rank * sizeof(double));
        ...
        poski_vec_t x_view = poski_CreateVec(x, size_rank, STRIDE_UNIT, NULL);
        poski_vec_t r_view = poski_CreateVec(r, size_rank, STRIDE_UNIT, NULL);
        poski_vec_t p_view = poski_CreateVec(p, size_rank, STRIDE_UNIT, NULL);
        poski_vec_t b_view = poski_CreateVec(b, size_rank, STRIDE_UNIT, NULL);
        poski_vec_t sol_view = poski_CreateVec(sol, size_rank, STRIDE_UNIT, NULL);
        poski_vec_t Ap_view = poski_CreateVec(Ap, size_rank, STRIDE_UNIT, NULL);
